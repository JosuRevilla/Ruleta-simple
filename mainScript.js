...
function renderValuesList() {
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        if (currentEdit && currentEdit.index !== i) {
            const existingEditRow = valuesList.querySelector('.edit-row');
            if (existingEditRow && currentEdit.original) {
            valuesList.replaceChild(currentEdit.original, existingEditRow);
            }
            currentEdit = null;
        }

        // Si ya estamos editando este mismo índice, no hacemos nada
        if (currentEdit && currentEdit.index === i) {
            // ya hay una edición abierta para este índice
            return;
        }
        ...
        cancelBtn.addEventListener('click', () => {
            ...
            if (editRowNow) valuesList.replaceChild(currentEdit.original, editRowNow);
            }
            currentEdit = null;
        });
...
addBtn.addEventListener('click', () => {
    ...
    // si hay una edición activa, cancelar antes de añadir (evita confusión visual)
    if (currentEdit) {
        const editRowNow = valuesList.querySelector('.edit-row');
